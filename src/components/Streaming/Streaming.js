...
import ImgVisor1Streaming from "../../images/visor1-streaming.PNG";
...
import Visor1Streaming from "./Visor1Streaming";

const Visor1UUID = "0dee56e3-bbc7-4a0c-a24f-2b8dacdaa03d"
...
export default function Streaming(props) {
    ...
    const [auth, setAuth] = useState(localStorage.getItem('auth-Streaming'));
    const [accessToken, setAccessToken] = useState(localStorage.getItem('token-Streaming'));
    const [username, setUsername] = useState(localStorage.getItem('username-Streaming'));
    ...
    async function rol(token, usuario) {
        ...
        res
            .json()
            .then(d => {
                if (d['roles'].includes('Administrador') || d['roles'].includes('Visor Streaming')) {
                    localStorage.setItem('token-Streaming', token);
                    localStorage.setItem('auth-Streaming', true);
                    localStorage.setItem('username-Streaming', usuario);
                    setUsername(usuario);
            ...
            <Toolbar>
                ...
                {auth ?
                    <React.Fragment>
                        ...
                        <IconButton edge='start' color='inherit' onClick={() => { localStorage.removeItem('token-Streaming'); localStorage.removeItem('auth-Streaming'); localStorage.removeItem('username-Streaming'); setAccessToken(''); setAuth(false); setUsername(''); }}>
                            <LogoutIcon />
                        </IconButton>
                    </React.Fragment>
                    :
    ...
    useEffect(() => {
        document.title = "Visualización CNC - Streaming"
    }, []);
                                ...
                                <Grid item xs={12} sm={6} md={4}>
                                    <Card style={{ height: "100%", display: "flex", flexDirection: "column" }}>
                                        <CardHeader
                                            ...
                                            title={
                                                <Typography variant="h6" style={{ color: "white", fontSize: 21 }}>Monitoreo streaming</Typography>
                                            }
                                        />
                                        <Divider />
                                        <CardContent style={{ padding: 8, display: "contents" }}>
                                            <img src={ImgVisor1Streaming} alt="visor1-streaming.PNG" />
                                        </CardContent>
