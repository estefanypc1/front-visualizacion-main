...
import ImgVisor1ColegioSJDLV from "../../images/visor1-ColegioSJDLV.PNG";
...
import Visor1ColegioSJDLV from "./Visor1ColegioSJDLV";

const Visor1UUID = "ff37b941-2381-41f6-8118-b22b0505c402"
...
export default function ColegioSJDLV(props) {
    ...
    const [auth, setAuth] = useState(localStorage.getItem('auth-ColegioSJDLV'));
    const [accessToken, setAccessToken] = useState(localStorage.getItem('token-ColegioSJDLV'));
    const [username, setUsername] = useState(localStorage.getItem('username-ColegioSJDLV'));
    ...
    async function rol(token, usuario) {
        ...
        res
            .json()
            .then(d => {
                if (d['roles'].includes('Administrador') || d['roles'].includes('Visor Colegio SJDLV')) {
                    localStorage.setItem('token-ColegioSJDLV', token);
                    localStorage.setItem('auth-ColegioSJDLV', true);
                    localStorage.setItem('username-ColegioSJDLV', usuario);
                    setUsername(usuario);
    ...
    useEffect(() => {
        document.title = "Visualización CNC - ColegioSJDLV"
    }, []);
                            ...
                            <Grid container direction="row" alignItems="stretch" spacing={2}>
                                <Grid item xs={12} sm={6} md={4}>
                                    <Card style={{ height: "100%", display: "flex", flexDirection: "column" }}>
                                        <CardHeader
                                            style={{ background: "#333457", paddingTop: 12, paddingBottom: 12 }}
                                            title={
                                                <Typography variant="h6" style={{ color: "white", fontSize: 21 }}>Apropiación Digial 4.0</Typography>
                                            }
                                        />
                                        <Divider />
                                        <CardContent style={{ padding: 8, display: "contents" }}>
                                            <img src={ImgVisor1ColegioSJDLV} alt="visor1-ColegioSJDLV"/>
                                        </CardContent>
                                        <CardActions style={{ justifyContent: "flex-end", paddingBottom: 12, paddingRight: 12 }}>
                                            <ThemeProvider theme={responsiveFontSizes(buttonColor('#333457'))}>
