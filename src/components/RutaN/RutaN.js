...
import LogoRutaN from "../../images/logo-rutan.png";
import ImgVisor1RutaN from "../../images/visor1-rutan.png";
...
import Visor1RutaN from "./Visor1RutaN";
import Visor2RutaN from "./Visor2RutaN";

const Visor1UUID = "5e595800-9363-439d-afc0-e667cc0e3c7d"
const Visor2UUID = "8e20fa66-1cc1-4ce5-9228-235776ac1bec"
...
export default function RutaN(props) {
    ...
    const [auth, setAuth] = useState(localStorage.getItem('auth-rutan'));
    const [accessToken, setAccessToken] = useState(localStorage.getItem('token-rutan'));
    const [username, setUsername] = useState(localStorage.getItem('username-rutan'));
    ...
    async function rol(token, usuario) {
        ...
        res
            .json()
            .then(d => {
                if (d['roles'].includes('Administrador') || d['roles'].includes('Visor RutaN')) {
                    localStorage.setItem('token-rutan', token);
                    localStorage.setItem('auth-rutan', true);
                    localStorage.setItem('username-rutan', usuario);
                    setUsername(usuario);
    ...
    const appBarCustom = (
        <ThemeProvider theme={actualTheme2}>
            <AppBar position="static" style={{ marginBottom: theme.spacing(2), background: "linear-gradient(to right, #bfcf1b, #b6c91e)" }}>
                <Toolbar>
                    ...
                    <div className={classes.logoSpacing2}>
                        <a style={{ display: "contents" }} href={"https://www.rutanmedellin.org/"}>
                            <img src={LogoRutaN} alt="logo-rutan" className={classes.logo2} />
                        </a>
                    </div>
    ...
    useEffect(() => {
        document.title = "Visualización CNC - Ruta N"
    }, []);
                        ...
                        <Container maxWidth="lg">
                            ...
                            <Grid item xs={12} sm={6} md={4}>
                                    <Card style={{ height: "100%", display: "flex", flexDirection: "column" }}>
                                        <CardHeader
                                            style={{ background: "linear-gradient(to right, #bfcf1b, #b6c91e)", paddingTop: 12, paddingBottom: 12 }}
                                            title={
                                                <Typography variant="h6" style={{ color: "white", fontSize: 21 }}>Factor no respuesta </Typography>
                                            }
                                        />
                                        <Divider />
                                        <CardContent style={{ padding: 8, display: "contents" }}>
                                            <img src={ImgVisor1RutaN} alt="visor1-rutan" />
                                        </CardContent>
                                        <CardActions style={{ justifyContent: "flex-end", paddingBottom: 12, paddingRight: 12 }}>
                                            <ThemeProvider theme={responsiveFontSizes(buttonColor('#bfcf1b'))}>
                                ...
                                <Grid item xs={12} sm={6} md={4}>
                                    <Card style={{ height: "100%", display: "flex", flexDirection: "column" }}>
                                        <CardHeader
                                            style={{ background: "linear-gradient(to right, #bfcf1b, #b6c91e)", paddingTop: 12, paddingBottom: 12 }}
                                            title={
                                                <Typography variant="h6" style={{ color: "white", fontSize: 21 }}>Base completa </Typography>
                                            }
                                        />
                                        <Divider />
                                        <CardContent style={{ padding: 8, display: "contents" }}>
